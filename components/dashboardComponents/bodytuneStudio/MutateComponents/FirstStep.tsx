...
// Initials
const firstStepValidationInitials: firstStepValidation = {
  height: {
    valid: null,
    validationMessage: "",
  },
...
const FirstStep = ({
  ...
  // State
  const [firstStepFields, setFirstStepFields] = useState<firstStepType>({
    height: personalInfo.height!.toString(),
    weight: personalInfo.weight!.toString(),
    experience: "",
  });
  const [firstStepValidation, setFirstStepValidation] =
    useState<firstStepValidation>(firstStepValidationInitials);
  ...
  const selectOnChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFirstStepFields((prev) => ({ ...prev, [name]: value }));
  };
        ...
        <div className="flex justify-center gap-6">
          <div className="min-w-[10px] max-w-[120px] relative">
            <Input
              name="weight"
              placeholder="Weight"
              state={firstStepFields.weight}
              type="text"
              label="Weight (kg)"
              onChange={onChange}
              autoComplete="off"
              valid={firstStepValidation.weight.valid}
            />
          </div>
          <div className="min-w-[10px] max-w-[120px] relative">
            <Input
              name="height"
              placeholder="Height"
              state={firstStepFields.height}
              type="text"
              label="Height (cm)"
              onChange={onChange}
              autoComplete="off"
              valid={firstStepValidation.height.valid}
            />
          </div>
        </div>
        <div
          style={{
            border:
              firstStepValidation.experience.valid === null ||
              firstStepValidation.experience.valid
                ? ""
        ...
        >
          ...
          <div className={`flex flex-col w-full gap-2 bg-primary`}>
            <select
              ...
              name="birthMonth"
              defaultValue={firstStepFields.experience}
            >
